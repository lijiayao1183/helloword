<template>
  <div id="left-list">
    ...
    <div class="list-item-container"
         :class="{'border-bottom-none':!isEdit}">
      <div class="scroll-small">
        <el-collapse :value="activeName"
                     accordion
                     class="accordion">
          <el-collapse-item v-for="type in filteredData"
                            :key="type.name"
                            :name="type.name"
                            :title="type.title"
                            :class="{'active-node':activeName == type.name}"
                            @click.native="collapseChange(type)">
            <template v-for="(item,index) in type.list">
              ...
              <template v-else>
                <el-tooltip :disabled="tipDisabled[index]"
                            :content="item[prop.name]"
                            :key="index">
                  <div @click.stop="listItemClick(item[prop.id])"
                       class="list-item"
                    ...
                    <template v-if="isEdit">
                      <i class="iconfont delete-icon"
                         @click.stop="deleteItem(item[prop.id], item[prop.name])"
                         v-html="operateIcon"></i>
                    </template>
                  </div>
                </el-tooltip>
              </template>
    ...
    <template v-if="notSnapshot">
      <div class="btn-create"
        ...
        <el-button type="primary"
                   @click="createItem">创建{{createLabel}}</el-button>
      </div>
    </template>
  </div>
</template>
